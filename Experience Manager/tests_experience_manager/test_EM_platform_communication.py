...
class TestEMPlatformCommunication(unittest.TestCase):
    ...
    def test_platform_handshake(self):
        ...
        with patch.object(PlatformCommunication.__wrapped__ , '_is_platform_online') as mock_is_platform_online:
            mock_is_platform_online.return_value = True
            em = ExperienceManager()
            em.start_platform_communication()
        self.assertTrue(True)
        self.assertEqual(responses.calls[1].request.body, b'{"text": "start of communication. name: Experience Manager"}')
        self.assertEqual(responses.calls[3].request.body, b'{"text": "request domain and problem"}')
        self.assertNotEqual(em.PDDL_domain_text, "")
        self.assertNotEqual(em.PDDL_problem_text, "")
        self.assertEqual(responses.calls[4].request.body, b'{"text": "received domain and problem. request links for communication"}')
